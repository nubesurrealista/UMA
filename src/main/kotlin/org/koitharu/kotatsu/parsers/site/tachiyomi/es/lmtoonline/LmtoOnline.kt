...
@TachiyomiSource("LMTO_ONLINE", "LMTO.online", "es")
class LmtoOnline : HttpSource() {

    override val name = "LMTO.online"

    override val baseUrl = "https://lmto.online"
    ...
    override fun fetchLatestUpdates(page: Int): Observable<MangasPage> =
        fetchSearchManga(page, "", FilterList(OrderFilter(listOf("" to "recents"))))
    ...
    private fun fetchMangas() {
        ...
        mangaCache = series?.mangas.orEmpty()
        cacheTimestamp = now
    }
    ...
    private fun searchMangaParse(page: Int, query: String, filters: FilterList): MangasPage {
        ...
        val demographic = filters.firstInstanceOrNull<DemographicFilter>()?.selected.orEmpty()
        val type = filters.firstInstanceOrNull<TypeFilter>()?.selected.orEmpty()
        val nsfw = filters.firstInstanceOrNull<NsfwFilter>()?.selected.orEmpty()
        ...
        val filteredMangas = mangaCache
            ...
            .let { sequence ->
                when (order) {
                    ...
                    "recents" -> sequence.sortedByDescending { it.latestChapterCreatedAt }
                    "views" -> sequence.sortedByDescending { it.totalViews }
                    else -> sequence
                }
            }
            .toList()
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        val result = response.extractNextJs<MangaDetails>() ?: throw Exception("Manga no encontrado")
